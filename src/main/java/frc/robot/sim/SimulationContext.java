...
/**
 * Responsible for coordinating periodic simulation updates. Provide a {@link BatterySim battery} and a set of
 * {@link MechanismSim mechanisms} to simulate, then call {@link #update(double)} to step the simulation forward. It is
 * recommended to call {@code update} in a robot's {@link TimedRobot#simulationPeriodic()} method.
 */
public class SimulationContext {
	...
	/**
	 * Gets the default simulation context. Useful for convenient simulation registration, but should not be used in unit
	 * tests. The simulation uses a nominal 12-volt battery with no capacity loss over time, i.e. it will always output
	 * exactly 12 volts when not under load.
	 */
	 ...
	 *
	 * @param battery
	 *            the battery the simulation should use.
	 */
	 ...
	 * @param <M>
	 *            the type of the mechanism simulation
	...
	/**
	 * Updates the simulation by moving it forward one timestep. This will update all the mechanism simulations currently
	 * in this context and update the robot's battery voltage to account for the voltage drop caused by the currents
	 * drawn by all the mechanisms. The voltage can be read via {@link RobotController#getBatteryVoltage()} or from a
	 * mechanism simulation's {@link MechanismSim#getBatteryVoltage()} convenience method.
	 *
	 * @param timestep
	 *            how far forward to move the simulation, in seconds.
	 */
	public void update(double timestep) {
		...
		// Update generic simulations after mechanisms; subsystem simulations, in
		// particular, should
		...
		// Calculate the new battery voltage. Mechanism simulations can read it via
		// getBatteryVoltage()
